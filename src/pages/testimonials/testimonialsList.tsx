      ...
      <div className="bg-white rounded-lg sm:rounded-xl shadow-md border border-[#433D8B]/50 overflow-hidden min-w-0 w-full">
        <div className="p-4 sm:p-6 lg:p-6 xl:p-8 border-b border-[#433D8B]/50">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 sm:gap-4 lg:gap-4 xl:gap-6">
            <h2 className="text-lg sm:text-xl lg:text-xl xl:text-2xl font-semibold text-[#17153B]">Testimonials</h2>
            <button
                    ...
                    <div className="flex gap-3">
                      {item.image ? (
                        <img
                          src={item.image}
                          alt={item.name}
                          className="w-12 h-12 rounded-full object-cover border-2 border-[#433D8B] shrink-0"
                        />
                      ) : (
                        <AvatarPlaceholder name={item.name} className="w-12 h-12 shrink-0" />
                      )}
                      <div className="min-w-0 flex-1">
                        <p className="font-medium text-[#17153B] text-sm">
                          {item.name}
                          {item.isOwner && (
                            <span className="ml-1 text-xs bg-[#C8ACD6] text-white px-1.5 py-0.5 rounded">Owner</span>
                          )}
                        </p>
                        <p className="text-[#17153B]/80 text-xs">{item.designation} {item.company && `· ${item.company}`}</p>
                      </div>
                    </div>
                    <p className="text-[#17153B]/80 text-xs mt-2 overflow-hidden max-h-12">{item.testimonial}</p>
                    <p className="text-[#17153B]/60 text-xs mt-2">{formatDate(item.createdAt)}</p>
                    <button
                ...
                <table className="w-full min-w-[1100px] text-left border-collapse table-auto">
                  <thead>
                    <tr className="bg-[#F0EBF5] border-b border-[#433D8B]">
                      <th className="px-3 py-2 sm:px-4 sm:py-3 text-sm font-semibold text-[#17153B] text-left">Image</th>
                      <th className="px-3 py-2 sm:px-4 sm:py-3 text-sm font-semibold text-[#17153B] text-left">Name</th>
                      <th className="px-3 py-2 sm:px-4 sm:py-3 text-sm font-semibold text-[#17153B] text-left">Designation</th>
                      <th className="px-3 py-2 sm:px-4 sm:py-3 text-sm font-semibold text-[#17153B] text-left">Company</th>
                      <th className="px-3 py-2 sm:px-4 sm:py-3 text-sm font-semibold text-[#17153B] text-left">Testimonial</th>
                  ...
                  <tbody>
                    {testimonials.map((item) => (
                      <tr key={item._id} className="border-b border-[#433D8B]/30 hover:bg-[#F0EBF5]/50 transition">
                        <td className="px-3 py-2 sm:px-4 sm:py-3">
                          {item.image ? (
                            <img src={item.image} alt={item.name} className="w-10 h-10 rounded-full object-cover border-2 border-[#433D8B]" />
                          ) : (
                        ...
                        <td className="px-3 py-2 sm:px-4 sm:py-3 align-top whitespace-nowrap">
                          <span className="flex items-center gap-1">
                            ...
                            {item.isOwner && <span className="shrink-0 text-xs bg-[#C8ACD6] text-white px-1.5 py-0.5 rounded">Owner</span>}
                          </span>
                        </td>
                        <td className="px-3 py-2 sm:px-4 sm:py-3 align-top whitespace-nowrap">
                          <span className="text-[#17153B]/80 text-sm">{item.designation ?? "—"}</span>
                        </td>
                        <td className="px-3 py-2 sm:px-4 sm:py-3 align-top whitespace-nowrap">
                          <span className="text-[#17153B]/80 text-sm">{item.company ?? "—"}</span>
                        </td>
                        <td className="px-3 py-2 sm:px-4 sm:py-3 align-top whitespace-nowrap max-w-[240px]">
                          <span className="block truncate text-[#17153B]/80 text-sm" title={item.testimonial}>{truncateText(item.testimonial, 80)}</span>
                        </td>
                        <td className="px-3 py-2 sm:px-4 sm:py-3 text-sm text-[#17153B]/70 whitespace-nowrap">{formatDate(item.createdAt)}</td>
                        <td className="px-3 py-2 sm:px-4 sm:py-3 pr-6">
                          <button type="button" onClick={() => openDetailModal(item._id)} className="shrink-0 px-3 py-1.5 rounded-lg bg-[#17153B] text-white hover:bg-[#17153B]/90 text-sm font-medium whitespace-nowrap">
                            View
                          </button>
                        </td>
                      </tr>
